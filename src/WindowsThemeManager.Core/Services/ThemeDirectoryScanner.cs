...
public class ThemeDirectoryScanner : Interfaces.IThemeDirectoryScanner
{
    ...
    /// <inheritdoc />
    public IEnumerable<string> ScanThemeDirectories(CancellationToken cancellationToken = default)
    {
        ...
        foreach (var directory in ThemeDirectoryPaths)
        {
            ...
            try
            {
                ...
                ScanDirectory(directory, themePaths, cancellationToken);

                // Also scan subdirectories (themes often have their own folders)
                foreach (var subDirectory in Directory.EnumerateDirectories(directory))
                {
                    cancellationToken.ThrowIfCancellationRequested();
                    ScanDirectory(subDirectory, themePaths, cancellationToken);
                }
            }
